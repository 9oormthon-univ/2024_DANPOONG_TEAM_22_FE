    ...
    return (
      ...
      android_ripple={{color: COLORS.blue600}}>
        ...
        <View className="flex-1">
          {/* 메뉴 제목 */}
          <View className="flex-row justify-start items-center gap-x-[11]"><Txt type="body3" text={title} className="text-white"/>{kakaoLogo && <KakaoLogo/>}</View>
          {/* 간격 조정 */}
          <View className="h-[4.9]" />
          {/* 메뉴 설명 */}
          {sub && <Txt type="caption1" text={sub} className="text-gray400" />}
        </View>
        {/* 화살표 아이콘 */}
