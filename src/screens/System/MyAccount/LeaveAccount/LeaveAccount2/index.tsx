...
import { type getVoicefilesRetentionResponse } from '@apis/DeleteAccount/get/VoicefilesRetention/type';
// apis
...
export const LeaveAccount2Screen = ({
  route,
}: {
  route: RouteProp<SystemStackParamList, 'LeaveAccount2'>;
}) => {
  ...
  const [voicefilesRetention, setVoicefilesRetention] = useState<
    getVoicefilesRetentionResponse['result']
  >({
    voiceCount: 0,
    thanksCount: 0,
    messageCount: 0,
      ...
      <View className="px-px" style={{ height: windowHeight - 196 }}>
        ...
        {role == 'HELPER' ? (
          <>
            ...
            <CustomText
              type="title4"
              text={`${youthMemberNum}명의 청년들이\n${nickname}님의 목소리를 들을 수 없게 돼요`}
            ...
            {/* 구분선 */}
            <View
              className="h-[5] bg-blue600"
              style={{ width: windowWidth, transform: [{ translateX: -30 }] }}
            />
            {/* dataView 영역*/}
            <FlexableMargin flexGrow={42} />
            <View className="rounded-tl-[10px] rounded-tr-[10px] w-full bg-blue500 pt-[28] pb-[23] px-px">
              <CustomText
                type="body3"
                text={`닉네임님의 정보, 활동 내역 등\n소중한 기록이 모두 사라져요`}
                className="text-white"
              />
              <View className="h-[11px]" />
              <CustomText
                type="caption2"
                text={`탈퇴하면 다시 가입하더라도 이전 정보를 되돌릴 수 없어요`}
            ...
            <View className="rounded-bl-[10px] rounded-br-[10px] w-full bg-blue600 pt-[30] pb-[5] px-px">
              {[
                {
                  icon: <DiaICon />,
                  text: '청년의 일상을 비추는 목소리',
                  count: voicefilesRetention.voiceCount,
                },
                {
                  icon: (
                    <View className="w-[22px] h-[22px]">
                      <LetterIcon className="text-blue400" />
                    </View>
                  ),
                  text: '청년에게 받은 편지',
                  count: voicefilesRetention.messageCount,
                },
                {
                  icon: <HeartIcon />,
                  text: '청년에게 받은 감사표현',
                  count: voicefilesRetention.thanksCount,
                },
              ].map((item, idx) => (
                <View
                  key={`${item.text}-${idx}`}
                  className="flex-row justify-between mb-[25px]">
          ...
          <>
            ...
            <CustomText
              type="body4"
              text={`계정 정보, 활동 내역 등 소중한 기록이 모두 사라져요\n탈퇴하면 다시 가입하더라도 이전 정보를 되돌릴 수 없어요`}
              className="text-gray300"
            />
