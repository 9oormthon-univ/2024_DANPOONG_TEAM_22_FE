...
type AlarmCategory = NonNullable<
  Parameters<typeof postAlarmSettingToggleByAlarmCategoryAndBool>[0]
>['alarmCategory'];
...
type NotificationConfig = {
  ...
  timeField: keyof Omit<
    MemberInfoYouthResponseType,
    | 'wakeUpAlarm'
    | 'sleepAlarm'
    | 'breakfastAlarm'
    | 'lunchAlarm'
    | 'dinnerAlarm'
    | 'outgoingAlarm'
  >; // API 응답에서 시간 정보를 가져올 키
  alarmField: keyof Omit<
    MemberInfoYouthResponseType,
    | 'wakeUpTime'
    | 'sleepTime'
    | 'breakfast'
...
const notificationsConfig: NotificationConfig[] = [
  {
    sub: '기상',
    alarmCategory: 'WAKE_UP',
    timeField: 'wakeUpTime',
    alarmField: 'wakeUpAlarm',
  },
  {
    sub: '날씨',
    alarmCategory: 'GO_OUT',
    timeField: 'outgoingTime',
    alarmField: 'outgoingAlarm',
  },
  {
    sub: '아침 식사',
    alarmCategory: 'MEAL_BREAKFAST',
    timeField: 'breakfast',
    alarmField: 'breakfastAlarm',
  },
  {
    sub: '점심 식사',
    alarmCategory: 'MEAL_LUNCH',
    timeField: 'lunch',
    alarmField: 'lunchAlarm',
  },
  {
    sub: '저녁 식사',
    alarmCategory: 'MEAL_DINNER',
    timeField: 'dinner',
    alarmField: 'dinnerAlarm',
  },
  {
    sub: '취침',
    alarmCategory: 'SLEEP',
    timeField: 'sleepTime',
    alarmField: 'sleepAlarm',
  },
];
...
export const NotificationSettingYouth = () => {
  ...
  const fetchMemberInfo = async () => {
    ...
    const fetchedNotifications = notificationsConfig.map(config => {
      ...
      const formattedHour = `${parsedTime.hour < 12 ? '오전' : '오후'} ${
  ...
  useEffect(() => {
    if (!showMinuteBottomSheet) {
      setNotifications(prev =>
        prev.map((notif, i) =>
          i === selectedIndex
            ? { ...notif, hour: tempHour, minute: tempMinute }
            : notif,
        ),
      );
    }
